...
pub struct SP1HeliosOperator<P> {
    ...
    provider: P,
    lightclient_pk: Arc<<EnvProver as Prover>::ProvingKey>,
    storage_slots_pk: Arc<<EnvProver as Prover>::ProvingKey>,
    ...
    P: Provider + WalletProvider,
{
    ...
    pub async fn new(
        ...
        let lightclient_pk = client
            .setup(Elf::Static(LIGHTCLIENT_ELF))
            .await
            .expect("Failed to set up light client program");
        tracing::info!("Setting up storage slots program...");
        let storage_slots_pk = client
            .setup(Elf::Static(STORAGE_ELF))
            .await
            .expect("Failed to set up storage slots program");
